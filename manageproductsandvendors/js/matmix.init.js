jQuery(document).ready(function ($) {
    ...
    * switchery.js
    ...
    //Message and Share Button
    $('.m-user-message').on('click', function (e) {
        e.preventDefault();
        var Ubox = bootbox.dialog({
            title: "Message for: Amery ",
            message:  
                ...
                '<div id="email" class="hidden-xs">Email</div>' +
                '</button>' +
                '</div>' +
                '<div class="btn-group" role="group">' +
                '<button type="button" id="favorites" class="btn btn-default" href="#tab2" data-toggle="tab"><span class="glyphicon glyphicon-comment" aria-hidden="true"></span>' +
                '<div class="hidden-xs">SMS</div> ' +
                '</button>' +
                '</div> ' +
                '</div>' +
                '<div class="well">' +
            ...
            buttons: {
                update: {
                    label: "Send",
                    className: "btn-success col-md-4 text-center",
                    callback: function () {
                        var name = $('#name').val();
                        var answer = $("input[name='awesomeness']:checked").val()
                        DemoCallBack.show("Send");
                    }
                } 
            }
        });
    ...
    $('.m-user-share').on('click', function (e) {
        e.preventDefault();
        var Ubox = bootbox.dialog({
            title: "Share",
            message:
